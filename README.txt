=========================================
      Downloads Sorter v1.0.0
=========================================

Thank you for using Downloads Sorter! This tool automatically organizes your 
cluttered Downloads folder into neat categories like Documents, Media, 
Installers, and more.

--- INSTALLATION ---
1. Extract ALL files from this ZIP folder to a new folder on your computer.
2. Double-click 'Installer.exe' to begin.
3. Once finished, you will find a "Downloads Sorter" shortcut on your desktop.

--- IMPORTANT: ANTIVIRUS WARNINGS ---
Because this is a custom-made app and not "digitally signed" with an expensive 
corporate certificate, Windows and Antivirus programs (like Norton or Defender) 
may flag it as "Unknown" or "Suspicious."

- If Windows shows a blue "Windows protected your PC" screen:
  Click "More info" and then "Run anyway."

- If Norton or other Antivirus flags it:
  Select "Always Allow," "Restore," or add the folder to your exclusions list.
  This is a safe, open-source script designed only to move your files.

--- HOW TO USE ---
Whenever your Downloads folder gets messy, just double-click the 
"Downloads Sorter" icon on your desktop. It runs invisibly and finishes 
in seconds!

--- FEATURES ---
- Sorts files into: Installers, Documents, Media, Archives, and To Review.
- Cleans up 'Temporary' files older than 14 days.
- Creates a 'SortDownloads.log' in your Downloads folder so you can see what moved.

Enjoy your clean Downloads folder!
